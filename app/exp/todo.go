...
type TodoCrudService interface {
...
type TodoCrudServiceImpl struct {
...
type TodoIndexualService interface{

	TodoByID(db XODB, id int) (*Todo, error)
}
...
type TodoIndexualService interface{
	...
	TodoByTitle(db XODB, title string) (*Todo, error)
}
...
type TodoIndexualService interface{
	...
	TodoByDueDate(db XODB, dueDate pq.NullTime) (*Todo, error)
}


// TodoByDueDate retrieves a row from 'public.todo' as a Todo.
//
// Generated from index 'uniqdue_date'.
func TodoByDueDate(db XODB, dueDate pq.NullTime) (*Todo, error) {
	var err error
	fmt.Println(, dueDate pq.NullTime)
	...
	// run query
	XOLog(sqlstr, dueDate)
	t := Todo{
		_exists: true,
	}

	err = db.QueryRow(sqlstr, dueDate).Scan(&t.ID, &t.Title, &t.Done, &t.DueDate)
	if err != nil {
		return nil, err
	}
