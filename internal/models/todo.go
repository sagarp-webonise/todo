...
func (t *Todo) Insert(db XODB) error {
	...
	// run query
	XOLog(sqlstr, t.Title, t.Done)
	err = db.QueryRow(sqlstr, t.Title, t.Done).Scan(&t.ID)
	if err != nil {
		return err
	}
...
func (t *Todo) Update(db XODB) error {
	...
	// run query
	XOLog(sqlstr, t.Title, t.Done, t.ID)
	_, err = db.Exec(sqlstr, t.Title, t.Done, t.ID)
	return err
}
...
func (t *Todo) Upsert(db XODB) error {
	...
	const sqlstr = `INSERT INTO public.todo (` +
		...
		`id, title, done` +
		`) = (` +
		`EXCLUDED.id, EXCLUDED.title, EXCLUDED.done` +
		`)`

	// run query
	XOLog(sqlstr, t.ID, t.Title, t.Done)
	_, err = db.Exec(sqlstr, t.ID, t.Title, t.Done)
	if err != nil {
		return err
	}
...
func GetAllTodos(db XODB) ([]*Todo, error) {
	...
	for q.Next() {
		...
		// scan
		err = q.Scan(&t.ID, &t.Title, &t.Done)
		if err != nil {
			return nil, err
		}
...
func TodoByID(db XODB, id int) (*Todo, error) {
	...
	err = db.QueryRow(sqlstr, id).Scan(&t.ID, &t.Title, &t.Done)
	if err != nil {
		return nil, err
	}
...
func TodoByTitle(db XODB, title string) (*Todo, error) {
	...
	err = db.QueryRow(sqlstr, title).Scan(&t.ID, &t.Title, &t.Done)
	if err != nil {
		return nil, err
	}
